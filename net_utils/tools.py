...
def convert_result_no_gt(data_batch, est_data):
    split = data_batch["split"]
    save_dict_list = []
    for idx, interval in enumerate(split):
        sequence_id = data_batch['sequence_id'][idx]
...
def total3d_todevice_inference_only(data, device):
    image = data['image'].to(device)
    sequence_id = data["sequence_id"]
    patch = data['boxes_batch']['patch'].float().cuda()
    g_features = data['boxes_batch']['g_feature'].float().cuda()
    ...
    object_input = {'patch': patch, 'g_features': g_features, 'size_cls': size_cls, 'box_feat': box_feat,
                    'split': split, 'rel_pair_counts': rel_pair_counts, 'bdb2D_pos': bdb2D, "bdb2D": bdb2D}
